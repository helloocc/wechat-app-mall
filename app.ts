import { API_CITY } from "./utils/livestart/api";
import { getPromise } from "./utils/livestart/net";
import { QrCodePath } from "./utils/livestart/const";
...
App({
    ...
    wx.getStorageInfo({
      success(res) {
        res.keys.forEach((element) => {
          console.log(element, wx.getStorageSync(element));
        });
      },
    });
  },
  ...
  // 获取城市列表数据，如果本地没有就发送请求
  // 返回 Promise；使用 await getApp().getCityList() 获取城市列表
  async getCityList(): Promise<City[]> {
    if (this.globalData.cityList?.length > 0) {
      // 当前 globalData 中已经存在值了，直接返回
      return Promise.resolve(this.globalData.cityList);
    }
    // 当前 globalData 中无数据，说明还没有加载过，发送请求。
    const cityList = await this.loadCityList();
    cityList.map((city, index) => {
      cityList[index].qrCode = QrCodePath(city.qrCode);
    });
  ...
  loadCityList(): Promise<City[]> {
    return getPromise<City[]>(API_CITY);
  },
  ...
  globalData: {
    ...
    cityId: userInfo.getUserCityId() || (0 as number),
    // cityId: wx.getStorageSync("cityId") || 0 as number, // 用户上次选择的城市 id，默认为 0
    cityList: <Array<City>>[], // 城市列表
  },
});
